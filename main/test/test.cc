...
void UdpBroadcaster::process_command_thread() {
    while (running_) {
        ...
        if (cmd->cmd == "auth") {
            ...
            auto& app = Application::GetInstance();
            app.SetMicTestMode(true);
            // 录音并同时播放，持续3秒
            ESP_LOGI("TEST", "Recording and playing for 3 seconds...");
            vTaskDelay(pdMS_TO_TICKS(5000)); // 等待3秒
            ESP_LOGI("TEST", "Recording and playback finished");
            app.SetMicTestMode(false);
